...
<body>
    ...
    <div class="container" id="ques">
      ...
      <?php
            ...
            $offset = ($currentPage - 1) * $itemsPerPage; // calculate offset for the SQL query

            $sql = "SELECT * FROM `threads` WHERE thread_cat_id=$id LIMIT $itemsPerPage OFFSET $offset";
            $result = mysqli_query($conn,$sql);
              ...
              while ($row = mysqli_fetch_assoc($result)) {
                ...
                echo '
                  <div class="d-flex my-3" id="main_div">
                    <img src="assets/images/user.png" alt="Loading...." class="me-3 rounded-circle" style="width: 60px; height: 60px;" />
                    <div>
                      <strong class="text-muted">Posted by : ' .$row2['user_email']. ' on ' .$date_time. '</strong>
                      <br/>
                      <div id="internal">
                        <h5><a href="Thread_spec.php?thread_id=' .$thread_id. '" 
                          id="decs" style="text-decoration: none;color:blue;">' .$cat_name. '</a></h5> 
                      </div> 
                ...
                $sql = "SELECT COUNT(*) AS total FROM `threads` WHERE thread_cat_id=$id";
                $result = mysqli_query($conn,$sql);
                ...
                for ($i = 1; $i <= $totalPages; $i++) {
                  echo '<a href="Threads.php?page=' . $i . '&catid=' . $id . '">' . $i . '</a> ';
                }
